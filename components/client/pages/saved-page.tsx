...
export async function ClientSavedPage({ locale }: { locale: ClientLocale }) {
  ...
  return <div><h1 className="text-3xl font-semibold">{locale === "fr" ? "Biens enregistrés" : "Saved Properties"}</h1><div className="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{saved.map((item) => <article key={item.projectId} className="group overflow-hidden rounded-[26px] border border-black/8 bg-[var(--color-soft-white)]"><Link href={`${locale === "fr" ? "/fr" : ""}/projects/${item.slug}`} className="block"><div className="relative aspect-[4/3] overflow-hidden bg-[var(--color-warm-ivory)]">{item.heroImageUrl ? <Image src={item.heroImageUrl} alt="" fill sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 33vw" className="object-cover transition-transform duration-500 ease-[var(--kh-ease-out)] group-hover:scale-[1.025]" /> : null}</div><div className="p-5 pb-3"><p className="text-xs uppercase tracking-[0.14em] text-[var(--color-teal)]">{item.category}</p><h2 className="mt-2 text-xl font-semibold">{item.title}</h2><p className="mt-2 text-sm text-[var(--color-stone)]">{item.location} · {item.bedroomsLabel}</p></div></Link><form action={removeSavedProjectAction} className="px-5 pb-5"><input type="hidden" name="locale" value={locale}/><input type="hidden" name="projectId" value={item.projectId}/><button type="submit" className="text-sm text-[var(--color-terracotta)] transition-colors hover:text-[var(--color-terracotta-deep)] active:scale-95">{locale === "fr" ? "Retirer" : "Remove"}</button></form></article>)}</div></div>;
}
