...
export function QuickDiscovery({ locale = "en" }: { locale?: KeyHoldLocale }) {
  ...
  return (
    <Reveal as="section" className="border-b border-black/[0.07] bg-[var(--color-champagne-soft)]">
      <form action={localizedHref("/discover", locale)} method="get" className="site-container grid gap-3 py-5 md:grid-cols-[minmax(0,1.5fr)_minmax(9rem,0.7fr)_minmax(11rem,0.8fr)_auto] md:items-end">
        <label>
          ...
          <input name="q" type="search" placeholder={copy.placeholder} className="min-h-12 w-full border border-black/10 bg-[var(--color-soft-white)] px-4 text-base outline-none transition-colors focus:border-[var(--color-teal)]" />
        </label>
        <label>
          <span className="mb-2 block text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-[var(--color-stone)]">{copy.route}</span>
          <select name="categories" defaultValue="" className="min-h-12 w-full border border-black/10 bg-[var(--color-soft-white)] px-3 text-base outline-none transition-colors focus:border-[var(--color-teal)] md:text-sm">
        ...
        <label>
          ...
          <select name="areaSlugs" defaultValue="" className="min-h-12 w-full border border-black/10 bg-[var(--color-soft-white)] px-3 text-base outline-none transition-colors focus:border-[var(--color-teal)] md:text-sm">
