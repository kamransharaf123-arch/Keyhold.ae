...
export function QuickDiscovery({ locale = "en" }: { locale?: KeyHoldLocale }) {
  ...
  return (
    <Reveal as="section" className="relative z-10 -mt-16 pb-16 sm:-mt-20 sm:pb-20 lg:-mt-24 lg:pb-24">
      <div className="site-container">
        <form action={localizedHref("/discover", locale)} method="get" className="kh-hero-float-card grid gap-3 border border-black/[0.06] bg-[var(--color-soft-white)] p-5 sm:p-7 md:grid-cols-[minmax(0,1.5fr)_minmax(9rem,0.7fr)_minmax(11rem,0.8fr)_auto] md:items-end">
          <label>
            <span className="mb-2 block text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-[var(--color-stone)]">{copy.search}</span>
            <input name="q" type="search" placeholder={copy.placeholder} className="min-h-12 w-full border border-black/10 bg-[var(--color-warm-ivory)] px-4 text-base outline-none transition-colors focus:border-[var(--color-teal)]" />
          </label>
          <label>
            <span className="mb-2 block text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-[var(--color-stone)]">{copy.route}</span>
            <select name="categories" defaultValue="" className="min-h-12 w-full border border-black/10 bg-[var(--color-warm-ivory)] px-3 text-base outline-none transition-colors focus:border-[var(--color-teal)] md:text-sm">
          ...
          <label>
            ...
            <select name="areaSlugs" defaultValue="" className="min-h-12 w-full border border-black/10 bg-[var(--color-warm-ivory)] px-3 text-base outline-none transition-colors focus:border-[var(--color-teal)] md:text-sm">
