...
export default async function AreasAdminPage({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  const translations = await Promise.all(areas.map((area) => getFrenchTranslation("area", area.slug)));
  return (
    <>
      <AdminPageHeader eyebrow="Taxonomy" title="Areas" description="Areas drive community landing pages, discovery filters and the schematic explorer." />
      <AdminNotice notice={query.notice} />
      <div className="grid gap-6">
        {areas.map((area, index) => (
          <AdminCard key={area.id} eyebrow="Area" title={area.name}>
            <AreaForm area={area} />
            <div className="mt-6 border-t border-black/10 pt-6">
              <EntityTranslationPanel
                ...
                fields={translationFieldProfiles.area ?? []}
                returnTo="/admin/areas"
              />
            </div>
          </AdminCard>
        ))}
        <AdminCard eyebrow="New" title="Add area"><AreaForm /></AdminCard>
      </div>
    </>
  );
}
